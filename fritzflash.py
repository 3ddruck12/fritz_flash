...
import sys
import time
from scp_upload import upload_to_fritzbox
...
def perform_bootloader_flash(sysupgradefile, imagefile, tftp_serve_name):
    # [...]
    print("Transfering sysupgrade target firmware")
    
    # NEU:
    upload_to_fritzbox(sysupgradefile)
...
def perform_bootloader_flash(
    ...
    with set_ip(ip_interface("192.168.1.70/24"), args.device) as can_set_ip:
        ...
        time.sleep(15)
        print("Transfering sysupgrade target firmware")
        upload_to_fritzbox(sysupgradefile, ip=target_host)
